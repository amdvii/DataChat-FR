...
from datachat.operations import classement_solde, flux_entre, solde_migratoire, top_destinations, top_origines
...
def test_arrondissements_regroupes(flux):
    ...
    #100 + 50 depuis 2 arrondissements de Paris vers Lyon -> un seul flux de 150
    ligne = flux[(flux['code_orig'] == '75056') & (flux['code_dest'] == '69123')]
    assert ligne['flux'].tolist() == [150]

    #les déménagements internes disparaissent
...
def test_resolution_commune(repertoire):
    ...
    #homonymes : la plus grosse par défaut, sinon celle du département demandé
...
def test_top_destinations(flux, repertoire):
    plan = Plan(operation='top_destinations', commune='Paris', annee=2018)
    res = top_destinations(flux, repertoire, plan)
...
def test_annee_par_defaut_la_plus_recente(flux, repertoire):
    plan = Plan(operation='top_destinations', commune='Paris')
    res = top_destinations(flux, repertoire, plan)
    assert res.attrs['contexte'].endswith('2020')


def test_top_origines(flux, repertoire):
    plan = Plan(operation='top_origines', commune='Cergy', annee=2020)
    res = top_origines(flux, repertoire, plan)
    assert res['commune'].tolist() == ['Saint-Denis']


def test_solde_migratoire(flux, repertoire):
    plan = Plan(operation='solde_migratoire', commune='Paris')
    res = solde_migratoire(flux, repertoire, plan)
    ligne_2018 = res[res['annee'] == 2018].iloc[0]
    assert (ligne_2018['arrivees'], ligne_2018['departs'], ligne_2018['solde']) == (50, 270, -220)


def test_flux_entre(flux, repertoire):
    plan = Plan(operation='flux_entre', commune='Paris', commune_b='Nantes')
    res = flux_entre(flux, repertoire, plan)
