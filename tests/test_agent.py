...
# le faux LLM renvoie juste les réponses de la liste, dans l'ordre


def creer_agent(flux, dossier, reponses):
    return DataChat(flux, llm=FakeListChatModel(responses=reponses), dossier_graphiques=dossier)
...
def test_pipeline_complet(flux, tmp_path):
    ...
    agent = creer_agent(flux, tmp_path, [plan, "En 2018, Lyon est la première destination."])
    rep = agent.ask("Où partent les Parisiens ?")
...
def test_le_llm_peut_corriger_un_plan_invalide(flux, tmp_path):
    ...
    agent = creer_agent(flux, tmp_path, [mauvais, bon, "Synthèse."])
    rep = agent.ask("Nantes attire-t-elle ?")

    assert rep.plan.operation == 'solde_migratoire'
    assert len(rep.erreurs) == 1 #1 erreur puis le LLM s'est corrigé


def test_question_hors_perimetre(flux, tmp_path):
    plan = '{"operation": "hors_perimetre", "raison": "Pas de données de prix."}'
    rep = creer_agent(flux, tmp_path, [plan]).ask("Prix du m2 à Nantes ?")
...
def test_commune_inconnue(flux, tmp_path):
    ...
    rep = creer_agent(flux, tmp_path, [plan]).ask("Qui arrive à Atlantide ?")
